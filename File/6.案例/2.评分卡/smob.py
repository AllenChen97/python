...
def smbin_cu(df,y,x,cutpoints=None,category=False,plot=True):
    ...
    cutpoints: cutpoints selected by the user.用户自定义的分割点
    ...
    out = pd.Series({"IVtable":IVtable,"IV":IV,"Xname":Xname,"Dict":Dict,"Xbin":bins})
...
def smbin(df,y,x,p=0.01,max_bin=20,plot=True):
    ...
    out_a = pd.Series({"Tree_structure":TS,"Cutpoints":CP})
